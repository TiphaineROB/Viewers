...
const circleRoiCursor = importInternal('tools/cursors');

const EVENTS = importInternal('events')
const getDiffBetweenPixelData = importInternal('util/segmentation')
const triggerLabelmapModifiedEvent = importInternal('util/segmentation')
const external = importInternal('externalModules')
const isToolActiveForElement = importInternal('store/isToolActiveForElement')
 ...
 */
class CustomBrush extends BaseTool {
  constructor(props) {
    const defaultProps = {
      name: CUSTOM_BRUSH,
      referencedToolName: CUSTOM_BRUSH,
