...
import {
    ...
    rotateMatrix902D,
    nearlyEqual
} from "./dcmjs_utilities/orientation/index.js";
import {
    encode,
    decode
} from "./dcmjs_utilities/compression/rleSingleSamplePerPixel.js";
import cloneDeep from "lodash.clonedeep";

import dcmjs from 'dcmjs';
const { DicomMessage, DicomMetaDictionary, BitArray, datasetToBlob } = dcmjs.data;
const { Normalizer } = dcmjs.normalizers;
const { log } = dcmjs.log;
const { Segmentation } = dcmjs.derivations;

const SegmentationCustom = {
    generateSegmentation,
    generateToolState,
    fillSegmentation
};

export default SegmentationCustom;
...
function _createSegFromImages(images, isMultiframe, options) {
    ...
    return new Segmentation([multiframe], options); // Default should be SegmentationDerivation
}
